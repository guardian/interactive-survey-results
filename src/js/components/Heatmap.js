export default class Heatmap {
	...
	_buildVisual() {
		...
		this.colorscales={
			...
			"font":d3.scale.linear().domain([17,17]).range(["#fff","#000"]),
		};
		
		//this.container.append("h2")
		//		.text(this.options.question.short || this.options.question.question)
...
class Legend {
	...
	_buildVisual() {
		...
		let titles=this.container.append("div")
						.attr("class","titles");
		titles.append("span").html("Surveyed people<br/>did better")
		titles.append("span").html("Guardian readers<br/>did better")
...
class TinyChart {
	...
	_buildVisual() {
		...
		//console.log(parent_width,this.options.left,delta)
		if(delta<0) {
			this.container.classed("right",true)
		}
		...
		this.container.append("p")
				.html((d)=>{
					...
					let the="";
					if(this.data.country==="UK" || this.data.ISO==="US" || this.data.ISO==="NL") {
						the="the ";
					}

					return `In the question about ${this.options.question.short||""} in ${the+this.data.country}, the Guardian readers ${how}`
				})
		...
		value.append("text")
				...
				.text(d=>{
					if(d==="mean") {
						return "IPSOS";this.data.country;
					}
					if(d==="actual") {
						return "Actual"
					}
		...
		value.append("text")
				...
				.text(d=>{
					if(d==="mean") {
						return "survey";//this.data.country;
					}
					if(d==="actual") {
						return "answer"
					}
